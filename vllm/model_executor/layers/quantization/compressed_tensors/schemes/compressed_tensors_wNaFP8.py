...
with FP8 (float8e4m3) symmetric input activation quantization (dynamic or
static, per-tensor). Requires SM >= 89 (Ada / Hopper). The activation quant
config is passed to the Humming kernel which applies it natively.
"""
...
__all__ = ["CompressedTensorsWNAFP8"]


class CompressedTensorsWNAFP8(CompressedTensorsScheme):
    ...
    def __init__(
        ...
        if num_bits not in WNA16_SUPPORTED_TYPES_MAP:
            raise ValueError(
                f"Unsupported num_bits = {num_bits} for WNAFP8; "
    ...
    def create_weights(
        ...
        **kwargs,
        ...
        if kernel_type.__name__ not in self._kernel_backends_being_used:
            logger.info("Using %s for CompressedTensorsWNAFP8", kernel_type.__name__)
