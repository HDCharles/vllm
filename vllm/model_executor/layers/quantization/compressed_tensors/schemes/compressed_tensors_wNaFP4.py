...
with FP4 (float4e2m1) symmetric dynamic per-token/per-group input
activation quantization. Requires SM >= 89 (Ada / Hopper); on hardware
below SM 120 the Humming kernel falls back to FP8 (float8e4m3) per-token
activations. Static, per-tensor, and asymmetric activation quantization
are not supported.
"""
...
__all__ = ["CompressedTensorsWNAFP4"]


class CompressedTensorsWNAFP4(CompressedTensorsScheme):
    _kernel_backends_being_used: set[str] = set()
    _fp4_fallback_warned: bool = False
    ...
    def __init__(
        ...
        if num_bits not in WNA16_SUPPORTED_TYPES_MAP:
            raise ValueError(
                f"Unsupported num_bits = {num_bits} for WNAFP4; "
        ...
        if input_quant is not None:
            if not input_quant.symmetric:
                raise ValueError(
                    "WNAFP4 requires symmetric activation quantization, "
                    f"got symmetric={input_quant.symmetric}"
                )
            if not input_quant.dynamic:
                raise ValueError(
                    "WNAFP4 requires dynamic activation quantization, "
            ...
            if input_quant.strategy not in (
                ...
                raise ValueError(
                    "WNAFP4 requires per-token or per-group activation "
    ...
    def create_weights(
        ...
        **kwargs,
        ...
        if kernel_type.__name__ not in self._kernel_backends_being_used:
            logger.info("Using %s for CompressedTensorsWNAFP4", kernel_type.__name__)
        ...
        if not CompressedTensorsWNAFP4._fp4_fallback_warned:
            cap = current_platform.get_device_capability()
            if cap is not None:
                sm = cap.major * 10 + cap.minor
                if sm < 120:
                    logger.warning(
                        "Model requests FP4 activation quantization but "
                        "current device (SM %d) does not support native FP4. "
                        "Humming kernel will use FP8 (float8e4m3) activations "
                        "instead.",
                        sm,
                    )
                    CompressedTensorsWNAFP4._fp4_fallback_warned = True
